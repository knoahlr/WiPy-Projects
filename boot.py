...
wiFiPass = {"landlord50preston":"50preston062016", "NoahS7":"Playstation4"}

for line in wlan.scan():

    NoahWiFi=re.search("landlord50preston|NoahS7",line[0])

    if NoahWiFi:

        NoahWiFi = NoahWiFi.group(0)
        wlan.connect(NoahWiFi, auth=(WLAN.WPA2, wiFiPass[NoahWiFi]), timeout=5000)
        time.sleep_ms(100)
        print("Connected to {0}".format(NoahWiFi))
        time.sleep(3)
        break
